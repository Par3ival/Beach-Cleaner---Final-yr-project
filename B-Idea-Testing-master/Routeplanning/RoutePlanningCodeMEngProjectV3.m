...
OxyVal= csvread('rpxvvalsv3.csv',1,0)
MAX_X=10;
MAX_Y=10;
MAX_VAL=10;
...
plot(xval+.5,yval+.5,'gd');% plot with green diamond for target
text(xval+1,yval+.5,'Rubbish')% labelling text 
% pause(2);
...
function i_min = min_fn(OPEN,OPEN_COUNT,xTarget,yTarget)
 ...
 if size(temp_array ~= 0)
  [i_min,temp_min]=min(temp_array(:,8));%Index of the smallest node in temp array
